<template>
  <div>
    <el-page-header @back="goBack">
      <template #content>
        <span class="text-large font-600 mr-3">新增客户信息</span>
      </template>
    </el-page-header>
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid: boolean) => {
    if (valid) {
      try {
        await useNuxtApp().$api('/cusBase', {
          method: 'POST',
          body: form
        })
        ElMessage({type: 'success', message: '追加が完了しました。', style: 'z-index: 3000;'});
        router.back()
      } catch (error) {
        ElMessage({type: 'error', message: '追加に失敗しました。', style: 'z-index: 3000;'});
      }
    }
  })
}
