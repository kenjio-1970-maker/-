<template>
  ...
  <el-table :data="tableData" style="width: 100%" v-loading="loading">
    <!-- 序号列，固定在左侧 -->
    <el-table-column v-if="!hideIndex" fixed type="index" label="No" width="43" header-align="center" align="center" />
    ...
    >
      <template #default="scope">
        ...
        <el-input 
          ...
          style="width: 100%" 
          :disabled="isReadOnly || scope.row.deliveryMod !== '9'" 
        />
        <el-input v-if="column.inTyp === 'textarea'" v-model="scope.row[column.prop]" size="medium" autosize type="textarea" class="full-width-input" :disabled="isReadOnly || scope.row.deliveryMod !== '9'" />
        <el-input v-if="column.inTyp === 'number'" v-model.number="scope.row[column.prop]" size="medium" class="full-width-input" :disabled="isReadOnly || scope.row.deliveryMod !== '9'"/>
        <el-input v-if="column.inTyp === 'money'" v-model.number="scope.row[column.prop]" size="medium" :formatter="formatMoney" :parser="parseMoney" class="full-width-input"  :disabled="isReadOnly || scope.row.deliveryMod !== '9'" />
        <elx-select 
          v-if="column.inTyp === 'select'" 
          v-model="scope.row[column.prop]" 
          :disabled="isReadOnly || scope.row.deliveryMod !== '9' || column.disabledParams"
        ...
        <el-date-picker 
          ...
          :disabled="isReadOnly  || scope.row.deliveryMod !== '9'"
        />
        <el-time-picker v-if="column.inTyp === 'time'" v-model="scope.row[column.prop]" size="medium" aria-label="Pick a time" style="width: 100%" :disabled="isReadOnly || scope.row.deliveryMod !== '9'" />
      </template>
    </el-table-column>
    ...
    <el-table-column v-if="!hideOperations" fixed="right" min-width="40" max-width="80" header-align="center" align="center">
      <template #default="scope">
        <!-- 删除按钮，点击后删除对应行 -->
        <el-button link type="primary" class="table-add-doc-btn-css" size="medium" @click.prevent="deleteRow(scope.$index)" :disabled="isReadOnly || scope.row.deliveryMod !== '9'">
          削除
        </el-button>
      </template>
    </el-table-column>
    <el-table-column v-if="!hideOperations && fixBtn1" fixed="right" min-width="112" max-width="160" header-align="center" align="center">
      <template #default="scope">
        <!-- 自定义按钮1 -->
    ...
    <el-table-column v-if="!hideOperations && fixBtn2" fixed="right" min-width="90" max-width="80" header-align="center" align="center">
      <template #default="scope">
        <!-- 自定义按钮2 -->
...
// 表格数据数组，使用 ref 使其具有响应性
let tableData = ref<any[]>([
  { deliveryMod: '1', sentDate: '2023-10-01', worker: 'John', fileName: 'file1.pdf', sendSts: '送信済み', firstColumn: '1' },
  { deliveryMod: '9', sentDate: '2023-10-02', worker: 'Jane', fileName: 'file2.pdf', sendSts: '未送信', firstColumn: '9' }
]);
...
/**
 * 添加新的表格行
 * 新行会包含 firstColumn 字段，默认为 '9'
 */
const onAddItem = () => {
  if (!tableData.value) {
    tableData.value = [];
  }
  tableData.value.push({ deliveryMod: '9', sentDate: dayjs(now) });
};
...
const handleSearch = async () => {
  ...
  try {
    ...
    // 更新表格数据
    tableData.value = tableResponse.data;  // hsxarm
  } catch (error) {
...
onMounted(() => {
  ...
  onPageLoadComplete(); // 调用页面加载完成后的方法
});
