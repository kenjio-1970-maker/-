  ...
  <el-table :data="tableData" style="width: 100%" v-loading="loading">
    ...
    <el-table-column v-for="(column, index) in tableColumns" :key="index" :prop="column.prop" :label="column.label" :width="column.width" header-align="center" align="center">
      <template #default="scope">
        <!-- 每个单元格都是一个输入框，支持数据编辑 -->
          <el-input v-if="column.inTyp === 'text'" v-model="scope.row[column.prop]" size="medium" class="full-width-input" style="width: 100%" :disabled="isReadOnly"/>
          ...
          <el-input 
            ...
            :disabled="isReadOnly"
          />
          <elx-select v-if="column.inTyp === 'select'" v-model="scope.row[column.prop]" :selectKey="typeof column.selectKey === 'function' ? column.selectKey(scope.row) : column.selectKey" :selectUrl="column.selectUrl"  :keyName = "column.keyName" :labelName = "column.labelName" style="width: 100%" :disabled="isReadOnly"/>
...
const onAddItem = () => {
  ...
  try{tableData.value.push({
    actionDate: dayjs(now).format('YYYY-MM-DD'),
  }) } catch (error) {
